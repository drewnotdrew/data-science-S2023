...
df_q1 <- df_michelson %>%
  ...
  summarize(n(), MeanVelocity = mean(Velocity))
...
**Observations**: - Write your observations here! - It seems as though
the mean velocity increases with distinctiveness, but the number of
observations of each distinctiveness goes down with distinctiveness. -
Why might your table differ from Michelson’s? - Rounding errors could
have changed the velocity values slightly, but in general they are
close.
...
uncertainty) greater or less than the true error? - The error between
the actual speed of light and his measurement was larger than Michelson
thought. Michelson was off by around 150m/s, while he though he was off
by +-51m/s. - Make a quantitative comparison between Michelson’s
uncertainty and his error. - Michelson was off my approximately three
times what he though he was, 152m/s and 51m/s respectively.
...
**Observations**: Similarities - The general trends are similar,
possible hinting that the model for the simulation has some truth to it.
The peaks and valleys occur on similar days. Differences - The simulated
mean velocities start out more consistent across days but end less
consistent across days, while the real mean velocities start less
consistent across days and become more consistent across days. In
general, for any given day, the real data seems more spread out, while
the simulated data is closer together. Perhaps there are outliers in the
real data that cause greater variability within a single day.
...
df_q2 %>%
  ggplot(aes(Date, Velocity)) +
  geom_point(
    mapping = aes(y = VelocityVacuum, shape = Distinctness, color = Temp)
  )
```

![](c02-michelson-assignment_files/figure-gfm/q5-more-visualizations-1.png)<!-- -->

``` r
df_q2 %>%
  group_by(Date) %>%
  summarize(mean_day_temp = mean(Temp), 
            mean_day_velocity = mean(VelocityVacuum), 
            mean_day_distinctness = mean(as.integer(Distinctness))
  ) %>%
  ggplot(aes(Date, Velocity)) +
  geom_point(
     mapping = aes(y = mean_day_velocity, color = mean_day_temp)
  )
```

![](c02-michelson-assignment_files/figure-gfm/q5-more-visualizations-2.png)<!-- -->
...
df_q2 %>%
  ...
  ggplot(aes(Date, Velocity)) +
  geom_point(
     mapping = aes(y = mean_day_velocity, color = mean_day_distinctness)
  )
```

![](c02-michelson-assignment_files/figure-gfm/q5-more-visualizations-3.png)<!-- -->

**Observations**:

- The first plot is a bit messy and not ideal, but we can still glean
  some information from it.
  - In general, the observations with the same distinctness seem to
    happened on the same day.

  - The distinctness does not seem to affect the velocity.

  - In general, the temperature seems to increase the velocity.
- The second plot is grouped by day for clarity. The first plot was
  messy, so this is an attempt to simplify the visualization.
  - This plot also suggests that the velocity increases with an increase
    in temperature, however it is clearer.
- The third plot graphs the mean velocity for each day where there were
  observations and color codes each point based on the mean distinctness
  for each day.
  - There does not seem to be a connection between distinctness and
    velocity, at least this plot does not suggest one.

  - The mean distinctness varies wildly between days according to plot
    3, but not within days according to plot 1. A deeper understanding
    of distinctness is needed in the context of this data set to draw
    further conclusions.
