...
-   Within the script editor or a chunk
    ...
    -   `Shift` + `Cmd/Ctrl` + `M` -\> insert pipe operator

    -   `Cmd/Ctrl` + `Enter` -\> run current line

    -   `F1` (Note: on a Mac you need to press `fn` + `F1`) -\> show
        help for function at cursor

    -   `Cmd/Ctrl` + `Shift` + `C` -\> comment/uncomment current
        line/selection

-   Within R Markdown

    -   `Cmd/Ctrl` + `Alt` + `I` -\> insert code chunk

-   Within a chunk

    -   `Shift` + `Cmd/Ctrl` + `Enter` -\> run current code chunk

    -   `Ctrl` + `I` (`Cmd` + `I`) -\> Reindent lines
