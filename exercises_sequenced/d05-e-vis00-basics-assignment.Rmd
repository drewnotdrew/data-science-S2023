...
-   Price goes up as carat goes up
-   Most fall within a triangle-like area, but there are outliers
...
## TODO: Complete this code
ggplot(diamonds) +
  geom_point(mapping = aes(x = carat, y = price, color = cut), size = 0.5)
```

**Observations**:

-   Price increases with carat
-   There a lot of data, so more attention will be needed to make this
    plot readable
-   Price seems more dependent on carat than the cut, though price still
    increases with cut.
