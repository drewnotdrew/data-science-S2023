...
Imagine we have some set of events $X$, and we're considering some *particul*

*ar* subset of cases that we're interested in $A$. For instance, imagine we're rolling a 6-sided die, and we're interested in cases when the number rolled is even. Then the subset of cases is $A = \{2, 4, 6\}$, and an *example run* of rolls might be $X = \{3, 5, 5, 2, 6, 1, 3, 3\}$.
...
-   What value of `fr` did you predict?
    -   The mean in the `rnorm` function is not specified, so I assume the default is simply zero.

    -   Edit: I misunderstood the frequency ratio. If the mean is 0, half of numbers should be negative and half should be positive. Thus, the frequency ratio should be close to 0.5.
-   Did your prediction match your calculation?
    -   No, but because I did not fully understand the frequency ratio. A fr of 0.5 makes sense given the sample and set.
...
-   What value does `fr` appear to be limiting towards?
    -   `fr` appears to be approaching 0.95.
