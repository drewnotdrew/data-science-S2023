...
*Reading*: [Scatterplots](https://rstudio.cloud/learn/primers/3.5) *Topics*: (All topics) *Reading Time*: \~40 minutes
...
mpg %>%
  ...
  geom_label_repel(data = df_annotate, aes(label = class, fill = class))
```
...
-   `hwy` tends to be higher - this makes sense because in general cars are more fuel efficent when they are traveling faster.

-   In general, `mpg` goes down the larger the engine is, up until engines become larger than 7 on the x axis.
