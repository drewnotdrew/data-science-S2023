...
-   Is there "one true value" for the material properties of Aluminum?
    -   No, there is some variation. However, a cursory glance at the data shows that the variation could be due to natural unaccounted for variables.
-   How many aluminum alloys are in this dataset? How do you know?
    -   The `n_distinct` function reports a value of 1 when fed the `alloy` column.
-   What angles were tested?
    -   The `n_distinct` function reports a value of 3 when fed the `angle` column.
...
-   The variation of Poission's ratio is quite small, small enough where it could be due to environmental factors unaccounted for in this dataset. This makes sense - the Poission Ratio of a material should not change based on the geometry of the part.
...
-   Does this graph support or contradict the claim above?
    -   Whether or not the graph supports the claim heavily depends on context on elasticity of materials. The units of elasticity are not mentioned for this dataset, but given that they are around 10,000, and the elasticity of aluminum is around 10,000 ksi, we can make an educated guess that the units are in ksi.
    -   The elasticity of steel is approximately 30,000 ksi, or roughly triple that of aluminum. The difference between these two is enough where variation smaller than 1000 ksi could be attributed to natural variation.
    -   While the graph supports the claim that elasticity is an intensive property given proper context, it does not display it convincingly. A sense of scale is needed, perhaps a line showing the average elasticity of the data, and a line for the typical elasticity of steel.
